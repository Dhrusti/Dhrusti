...
namespace WaltCapitalManagementWebAPI.Filters
{
    public class ActionFilter : IActionFilter
    {
        ...
        public ActionFilter(IConfiguration configuration, WaltCapitalDBContext dbContext, CommonHelper commonHelper, AuthRepo authRepo)
        {
                ...
                if (result is ObjectResult json)
                {
                    ...
                    if (!string.IsNullOrEmpty(data) && data != "GetDecryption" && data != "GetEncryption")
                    {
                        ...
                        if (isLogEnabled)
                        {
                            ...
                            _commonHelper.AddActivityLogUpdated(requestData.Path, requestData.Method, jsonDataString1, requestBody);
