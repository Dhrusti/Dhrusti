    ...
    public class FundDynamicInputPriceBLL
    {
        ...
        public CommonResponse AddUpdateFundDynamicInputPrice(int FundId, string UnitType, string Label, decimal Value, DateTime BalanceDate, int UpdatedBy, bool IsAddedFromPricing)
        {
            ...
            try
            {
                ...
                var FundInputPricingDetail = _commonRepo.GetFundDynamicInputPriceList(FundId).FirstOrDefault(x => x.BalanceDate.Date == BalanceDate.Date && x.Label.ToLower() == Label.ToLower() && x.UnitType.ToLower() == UnitType.ToLower() && x.IsAddedFromPricing==IsAddedFromPricing);
