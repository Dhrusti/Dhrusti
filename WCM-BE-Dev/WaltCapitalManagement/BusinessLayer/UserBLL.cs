    ...
    public class UserBLL
    {
            ...
            try
            {
                if (addClientPhase1ReqDTO != null)
                {
                    using (var scope = new TransactionScope())
                    {
                        ...
                        IsExistclient = _commonRepo.ALLUserList().Where(x => x.ClientAccNo == addClientPhase1ReqDTO.ClientAccNo && x.AccessCategoryId == 2).ToList();
                        ...
                        else
                        {
                            ...
                            commonResponse.Message = "User Account No is already taken!";
                        }
                    }
                }
                else
            ...
            try
            {
                if (updateClientPhase1ReqDTO != null)
                {
                    using (var scope = new TransactionScope())
                    {
                        ...
                        var IsExistclientdetails = _commonRepo.ALLUserList().Where(x => x.IsActive == true && !x.IsDeleted && x.Id != updateClientPhase1ReqDTO.Id && x.ClientAccNo == updateClientPhase1ReqDTO.ClientAccNo && x.AccessCategoryId == 2).ToList() ?? new List<UserMst>();
        ...
        public CommonResponse GetClientAllDetailById(GetClientAllDetailByIdReqDTO getClientAllDetailByIdReqDTO)
        {
            ...
            try
            {
                ...
                GetClientAllDetailByIdResDTO getClientAllDetailByIdResDTO = new GetClientAllDetailByIdResDTO();
                getClientAllDetailByIdResDTO.ClientDetail = new ClientDetail();
                getClientAllDetailByIdResDTO.ClientPortfolioServiceProviders = new List<ClientPortfolioServiceProvider>();
                getClientAllDetailByIdResDTO.ClientPortfolioInvestmentDetail = new ClientPortfolioInvestmentDetail();

                var UserDetails = _commonRepo.getUserList().Where(x => x.AccessCategoryId == ClientAccessCategoryId && x.Id == getClientAllDetailByIdReqDTO.ClientId).FirstOrDefault();

                if (UserDetails != null)
                {
                    ClientDetail clientDetail = new ClientDetail();
                    clientDetail.ClientId = UserDetails.Id;
                    clientDetail.PhotoPath = UserDetails.ProfilePhoto;
                    clientDetail.ClientName = UserDetails.FirstName + " " + UserDetails.LastName;
                    clientDetail.AccountNo = UserDetails.ClientAccNo;
                    clientDetail.BirthDate = UserDetails.Dob.ToString("dd MMMM yyyy");
                    clientDetail.JoiningDate = UserDetails.CreatedDate.ToString("dddd, dd MMMM yyyy");
                    clientDetail.Mobile = UserDetails.MobileNo;
                    clientDetail.Phone = UserDetails.WorkNo;
                    clientDetail.Email = UserDetails.Email;

                    var ServiceProviderList = _commonRepo.serviceProviderList();
                    var ServiceProviderTypeList = _commonRepo.serviceProviderTypeList();

                    var ExternalAccountList = _commonRepo.externalAccountList().Where(x => x.ClientId == getClientAllDetailByIdReqDTO.ClientId).ToList();
                    ...
                    List<ClientPortfolioServiceProvider> clientPortfolioServiceProviders = new List<ClientPortfolioServiceProvider>();
                        ...
                        if (!string.IsNullOrEmpty(Convert.ToString(item.ServiceProviderId)))
                        {
                            ...
                            foreach (var subItem in AccountCodeandType)
                            {
                                ...
                                if (CSVTotalAmount > 0)
                                {
                                    ...
                                    clientPortfolioServiceProviders.Add(new ClientPortfolioServiceProvider
                                    {
                                        ServiceProviderId = Convert.ToInt32(item.ServiceProviderId),
                                        ServiceProviderName = ServiceProviderName ?? string.Empty,
                                        ServiceProviderTypeId = subItem?.Type ?? 0,
                                        ServiceProviderTypeName = ServiceProviderTypeName ?? string.Empty,
                                        CurrencyShortName = "ZAR",
                                        TotalAmount = CSVTotalAmount ?? 0,
                                        TotalAmountString = _commonHelper.GetFormatedDouble(CSVTotalAmount ?? 0),
                                        ClientCount = 1,
                                        InvestedPercentage = 0,
                                        InvestedPercentageString = "N/A",
                                    });
                                }
                            }
                        }
                    ...
                    ClientPortfolioInvestmentDetail clientPortfolioInvestmentDetail = new ClientPortfolioInvestmentDetail();

                    var TotalInvestment = clientPortfolioServiceProviders.Sum(x => x.TotalAmount);

                    double PriceInUsd = _commonHelper.GetConvertedCurrency(CurrencySymbolConstant.South_African_Rand, CurrencySymbolConstant.United_States_Dollar, TotalInvestment, null).Data;
                    double GoldInOz = _commonHelper.GetConvertedCurrency(CurrencySymbolConstant.South_African_Rand, CurrencySymbolConstant.Gold_troy_ounce, TotalInvestment, null).Data;
                    ...
                    double PriceInBitcoin = _commonHelper.GetConvertedCurrency(CurrencySymbolConstant.South_African_Rand, CurrencySymbolConstant.Bitcoin, TotalInvestment, null).Data;

                    clientPortfolioInvestmentDetail.PriceInZar = "R " + TotalInvestment;
                    clientPortfolioInvestmentDetail.PriceInUsd = "USD " + _commonHelper.GetFormatedDouble(PriceInUsd);
                    clientPortfolioInvestmentDetail.PriceInGold = _commonHelper.GetFormatedDouble(PriceInGold) + " grams " + "(" + Convert.ToString(GoldInOz) + " Oz)";
                    clientPortfolioInvestmentDetail.PriceInBitcoin = "BTC " + _commonHelper.GetFormatedDouble(PriceInBitcoin);

                    getClientAllDetailByIdResDTO.ClientDetail = clientDetail;
                    getClientAllDetailByIdResDTO.ClientPortfolioServiceProviders = clientPortfolioServiceProviders;
                    getClientAllDetailByIdResDTO.ClientPortfolioInvestmentDetail = clientPortfolioInvestmentDetail;

                    commonResponse.Status = true;
                    commonResponse.StatusCode = HttpStatusCode.OK;
                    commonResponse.Message = "Success.";
                    commonResponse.Data = getClientAllDetailByIdResDTO;
                }
                else
                {
                    commonResponse.StatusCode = HttpStatusCode.NotFound;
