        ...
        public IQueryable GetRoleBasedData(IQueryable source, string? term = "", bool isValidate = true)
        {
            ...
            if (isDataSlicingOn)
            {
                ...
                if (roleName == Roles.Branch_Manager.ToString().Replace("_", " "))
                {
                    ...
                    if (elementType.Name == "CountryCustomMst")
                    {
                        var1 = Convert.ToString(roleDetails.UserMst.Country);
                        filterExpr.Append(filterExpr.Length > 0 ? filterExpr.Append(" && CountryId.Equals(" + var1 + ")") : "CountryId.Equals(" + var1 + ")");
                        term += term!.Concat("CountryId");
                    }
                    if (elementType.Name == "StateCustomMst")
                    {
                        var1 = Convert.ToString(roleDetails.UserMst.Province);
                        filterExpr.Append(filterExpr.Length > 0 ? filterExpr.Append(" && StateId.Equals(" + var1 + ")") : "StateId.Equals(" + var1 + ")");
                        term += term!.Concat("StateId");
                    }
                    if (elementType.Name == "CityCustomMst")
                    {
                        var1 = Convert.ToString(roleDetails.UserMst.City);
                        filterExpr.Append(filterExpr.Length > 0 ? filterExpr.Append(" && CityId.Equals(" + var1 + ")") : "CityId.Equals(" + var1 + ")");
                        term += term!.Concat("CityId");
                    }
                    if (elementType.Name == "OfficeMst")
                    {
                        var1 = Convert.ToString(roleDetails.UserMst.Office);
                        filterExpr.Append(filterExpr.Length > 0 ? filterExpr.Append(" && Id.Equals(" + var1 + ")") : "Id.Equals(" + var1 + ")");
                        term += term!.Concat("Id");
                    }
                    if (elementType.Name == "UserMst")
                    {
                        var1 = Convert.ToString(roleDetails.UserMst.Office);
