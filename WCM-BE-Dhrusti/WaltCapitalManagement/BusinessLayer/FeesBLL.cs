            ...
            try
            {
                ...
                if (getIFAFeeBreakdownReqDTO.FundId != null && getIFAFeeBreakdownReqDTO.FundId != 0)
                {
                    ...
                    foreach (var item in IfaBreakdownList)
                    {
                        ...
                        //TotalIFAAssets = IfaBreakdownList.Select(x => Convert.ToDecimal(x.TotalZARFee)).Sum();
                    }
                }
                else
                {
                    ...
                    var fundList = _commonRepo.fundList().Where(x => x.IsActive == true).ToList() ?? new List<FundMst>();

                    var activeClientTransactionList = (from fund in fundList
                                                       join ctm in _commonRepo.clientTransactionList() on fund.Id equals ctm.Fund
                                                       select ctm).ToList();

                    var clientTransactionList = (from ifaUl in IfaList
                                                 join ctm in activeClientTransactionList on ifaUl.Id equals ctm.Ifa
                                                 select ctm).ToList();
                    ...
                    foreach (var item in clientTransactionIfaDistinct)
                    {
                        ...
                        foreach (var subItem in clientTransactionsFundDistinct)
                        {
                            ...
                            {
                                OtherFee = otherFeePercentage;
                            }
                            ifaFeeBreakdown.ZARFee = _commonHelper.GetFormatedDecimal(Convert.ToDecimal(ZARFee));
                            ifaFeeBreakdown.VAT = VAT > 0 ? VAT.ToString("###0.00") : "0.00"; //_commonHelper.GetFormatedDecimal(VAT > 0 ? Convert.ToDecimal(VAT.ToString("#,##0.00")) :0); 
                            ifaFeeBreakdown.TotalZARFee = _commonHelper.GetFormatedDecimal(Convert.ToDecimal(ifaFeeBreakdown.TotalZARFee) + Convert.ToDecimal(TotalZARFee));
                            ifaFeeBreakdown.USDFee = _commonHelper.GetFormatedDecimal(Convert.ToDecimal(ifaFeeBreakdown.USDFee) + Convert.ToDecimal(OtherFee));
                        }
                        ...
                        ifaFeeBreakdown.ClientAccNo = IfaDetails.ClientAccNo;
                        ifaFeeBreakdown.AUM = IfaAUM > 0 ? IfaAUM.ToString("###0.00") : "0.00";  //_commonHelper.GetFormatedDecimal(IfaAUM);
                ...
                if (getIFAFeeBreakdownReqDTO.Alphabet != null && !string.IsNullOrEmpty(getIFAFeeBreakdownReqDTO.Alphabet))
                {
                    IfaBreakdownList = IfaBreakdownList.Where(x => x.LastName.ToLower().StartsWith(getIFAFeeBreakdownReqDTO.Alphabet.ToLower())).ToList();

                    getIFAFeeBreakdownResDTO.TotalCount = IfaBreakdownList.Count();
                }
                if (getIFAFeeBreakdownReqDTO.SearchString != null && !string.IsNullOrEmpty(getIFAFeeBreakdownReqDTO.SearchString))
                {
                    IfaBreakdownList = IfaBreakdownList.Where(x => x.FirstName.ToLower().Contains(getIFAFeeBreakdownReqDTO.SearchString.ToLower()) || x.LastName.ToLower().Contains(getIFAFeeBreakdownReqDTO.SearchString.ToLower()) || Convert.ToString(x.ClientAccNo.ToLower()).Contains(getIFAFeeBreakdownReqDTO.SearchString.ToLower())).ToList();
                    getIFAFeeBreakdownResDTO.TotalCount = IfaBreakdownList.Count();
                }
                ...
                decimal TotalIFAAssets = IfaBreakdownList.Select(x => Convert.ToDecimal(x.TotalZARFee)).Sum();
                getIFAFeeBreakdownResDTO.TotalIFAAssets = "R " + _commonHelper.GetFormatedDecimal(TotalIFAAssets);
