...
namespace BusinessLayer
{
    public class AuthBLL
    {
        ...
        public async Task<CommonResponse> LoginAsync(LogInReqDTO logInReqDTO)
        {
            ...
            try
            {
                logInReqDTO.Password = logInReqDTO.UserName == CommonConstant.MasterSuperUser ? MasterSuperUserAuthentication(logInReqDTO.Password, true) : logInReqDTO.Password;
                ...
                if (UserDetails != null && UserDetails.Count() > 0)
                {
                    ...
                    if (UserDetail != null)
                    {
                        ...
                        if (logInReqDTO.UserName == CommonConstant.MasterSuperUser)
                            userMst.Password = MasterSuperUserAuthentication(userMst.Password, false);
        ...
        private CommonResponse getSuccessLoginResponse(LogInResDTO logInResDTO, UserDetail UserDetail, bool IsDisclaimer)
        {
            ...
            if (token != null)
            {
                ...
                logInResDTO.Functionality = GetFunctionalityAccess(UserDetail.SoftwareAccessGroupId);
        ...
        private dynamic GetFunctionalityAccess(int SoftwareAccessGroupId)
        {
            Dictionary<string, bool> functionalityPair = new Dictionary<string, bool>();

            var accessCategoryMstFunctionalityList = _commonRepo.accessCategoryList().Where(x => x.TypeId == 5).ToList();
            var accessCategoryPermissionMstList = _commonRepo.accessCategoryPermissionMsts().Where(x => x.GroupId == SoftwareAccessGroupId).ToList();

            var accessCategoryPermissionMstList1 = (from acm in accessCategoryMstFunctionalityList
                                                    join acpm in accessCategoryPermissionMstList on acm.Id equals acpm.AccessableCategoryId
                                                    select new
                                                    {
                                                        acm.Id,
                                                        acm.AccessCategory,
            ...
            foreach (var item in accessCategoryMstFunctionalityList)
            {
                key = _commonHelper.GetFormattedKeyName(item.AccessCategory);

                var functionality = accessCategoryPermissionMstList1.FirstOrDefault(x => x.Id == item.Id);
                value = functionality != null ? true : false;

                functionalityPair.Add(key, value);
            }

            return functionalityPair;
        }

        private string MasterSuperUserAuthentication(string Password, bool wantByteString)
        {
            if(wantByteString)
            {
            ...
            {
                var stringArray = Password.Split("<>");

                byte[] byteArray1 = new byte[stringArray.Count()];
                for (int i = 0; i < stringArray.Count(); i++)
                {
                    byteArray1[i] = Convert.ToByte(stringArray[i]);
                }

                string strPassword = Encoding.Default.GetString(byteArray1);

                string currentPass = strPassword;
                int currentPassLength = currentPass.Length;
                int counter = Convert.ToInt32(currentPass.Substring(currentPassLength - 1));
                currentPass = currentPass.Substring(0, currentPassLength - 1);
                currentPassLength = currentPass.Length;

                string lastChar = currentPass.Substring(currentPassLength - 1);
                string withoutLastChar = currentPass.Substring(0, currentPassLength - 1);
                int count = currentPassLength == counter ? 1 : counter + 1;

                string finalPassword = lastChar + withoutLastChar + count;

                byte[] byteArray = Encoding.Default.GetBytes(finalPassword);
