    ...
    public class ClientTransactionBLL
    {
        ...
        public CommonResponse GetFundForCTByFundId(GetFundForCTByFundIdReqDTO getFundForCTByFundIdReqDTO)
        {
            ...
            try
            {
                ...
                if (FundDetail != null)
                {
                    ...
                    //if (FundDetail.PerformanceFeeA > 0 && FundDetail.ManagementFeeA > 0)
                    //    unitType.Add("Unit A");
                    //if (FundDetail.PerformanceFeeB > 0 && FundDetail.ManagementFeeB > 0)
                    //    unitType.Add("Unit B");
                    ...
                    unitType1 = _fundDynamicFieldBLL.GetUnitTypeList(FundDetail.Id);
        ...
        public CommonResponse GetTranscationTypeByClientId(GetTranscationTypeByClientIdReqDTO getTranscationTypeByClientIdReqDTO)
        {
            ...
            try
            {
                ...
                {
                    var clientTransactionDetails = _commonRepo.clientTransactionList().Where(x => x.Client == getTranscationTypeByClientIdReqDTO.ClientId && x.Fund == getTranscationTypeByClientIdReqDTO.FundId && x.UnitType.ToLower() == getTranscationTypeByClientIdReqDTO.UnitType.ToLower() && x.TransactionDate.Date <= getTranscationTypeByClientIdReqDTO.TranscationDate.Date).OrderBy(x => x.TransactionDate).ThenBy(x => x.Id).LastOrDefault();
                    ...
                    var pricevalue = GetPricingList.Where(x => x.UnitType.ToLower() == getTranscationTypeByClientIdReqDTO.UnitType.ToLower() && x.TransactionDate.Value.Date == getTranscationTypeByClientIdReqDTO.TranscationDate.Date).OrderByDescending(x => x.TransactionDate).FirstOrDefault();
                    if (clientTransactionDetails != null)
                    {
                        getTranscationTypeByClient.IsFirstTransaction = false;
                        getTranscationTypeByClient.NoOfUnit = pricevalue != null ? pricevalue.Units : 0;
                        getTranscationTypeByClient.UnitPrice = Convert.ToString(pricevalue != null ? _commonHelper.GetFormatedDecimal(pricevalue.UnitPriceNav) : 0);
                    }
                    else
                    {
                        getTranscationTypeByClient.IsFirstTransaction = true;
        ...
        public CommonResponse AddClientTransaction(AddClientTransactionReqDTO addClientTransactionReqDTO)
        {
            ...
            try
            {
                ...
                if (addClientTransactionReqDTO.TransactionDate.Date >= fund.InceptionDate.Date)
                {
                    if (addClientTransactionReqDTO.UnitPrice > 0 || addClientTransactionReqDTO.NumberOfUnits > 0)
                    {
                        if (addClientTransactionReqDTO.TransactionAmount > 0 && addClientTransactionReqDTO.NumberOfUnits > 0)
                        {
                            if (addClientTransactionReqDTO.Client != 0)
                            {
                                //var IsExistClient = _commonRepo.clientTransactionList().Where(x => x.Client == addClientTransactionReqDTO.Client && x.Fund == addClientTransactionReqDTO.Fund && x.TransactionDate.Date<= addClientTransactionReqDTO.TransactionDate.Date).OrderBy(x => x.TransactionDate).ThenBy(x => x.Id).LastOrDefault();
                                        ...
                                        if (ValidTransactionUnit)
                                        {
                                            ...
                                            using (var scope = new TransactionScope())
                                            {
                                                if (addClientTransactionReqDTO != null)
                                                {
                                                    ...
                                                    //decimal DynamicInputPricingAmount = _commonRepo.GetFundDynamicInputPriceList(fund.Id).Where(x => x.UnitType.ToLower() == clientTransactionMst.UnitType.ToLower() && x.Label.ToLower() == clientTransactionMst.AllocateTo.ToLower() && x.BalanceDate.Date == clientTransactionMst.TransactionDate.Date && x.IsAddedFromPricing == false).Select(x => x.Value).Sum();

                                                    //DynamicInputPricingAmount = DynamicInputPricingAmount + Convert.ToDecimal(clientTransactionMst.TransactionAmount);
                                                    //_fundDynamicInputPriceBLL.AddUpdateFundDynamicInputPrice(clientTransactionMst.Fund, clientTransactionMst.UnitType, clientTransactionMst.AllocateTo, DynamicInputPricingAmount, clientTransactionMst.TransactionDate, clientTransactionMst.CreatedBy, false);

                                                    _fundPricingBLL.UpdatePricingUsingTransaction(clientTransactionMst.Fund, clientTransactionMst.TransactionDate, Convert.ToDecimal(clientTransactionMst.TransactionAmount), clientTransactionMst.AllocateTo, clientTransactionMst.UnitType, clientTransactionMst.CreatedBy);
                        ...
                        {
                            commonResponse.Message = "Please Enter Valid Transaction Amount or NumberOfUnit!";
                            commonResponse.StatusCode = HttpStatusCode.BadRequest;
                        }
                    }
                    else
                    {
                        commonResponse.Message = "Invalid Unit Price or NumberOfUnit!";
                        commonResponse.StatusCode = HttpStatusCode.BadRequest;
                    }
                }
                else
