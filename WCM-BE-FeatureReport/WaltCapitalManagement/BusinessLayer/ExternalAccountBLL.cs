...
namespace BusinessLayer
{
    public class ExternalAccountBLL
    {
        ...
        public CommonResponse AddExternalAccount(AddExternalAccountReqDTO addExternalAccountReqDTO)
        {
            ...
            try
            {
                var IsExistAccount = _commonRepo.externalAccountList().Where(x => x.ServiceProvider == addExternalAccountReqDTO.ServiceProvider && x.Type == addExternalAccountReqDTO.Type && x.ClientId == addExternalAccountReqDTO.ClientId).ToList();
                if (IsExistAccount.Count == 0)
                {
        ...
        public CommonResponse UpdateupdateExternalAccount(UpdateExternalAccountReqDTO updateExternalAccountReqDTO)
        {
            ...
            try
            {
                ExternalAccountDetail externalAccountDetails = _dbContext.ExternalAccountDetails.Where(x => x.Id == updateExternalAccountReqDTO.Id).FirstOrDefault();
        ...
        public CommonResponse GetAllReportData(int ServiceProviderId, int ServiceProviderTypeId)
        {
            ...
            try
            {
                ...
                foreach (var item in ClientFilteredList)
                {
                    ...
                    //reportsResDTO.AccountValue = item.
                    //reportsResDTO.InvestmentValue = item.
                    //reportsResDTO.PortfolioValue = item.
